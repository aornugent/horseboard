...
export class ExpiryScheduler {
  ...
  private displayRepo: Repository<'displays'> | null = null;
  private horseRepo: Repository<'horses'> | null = null;
  private onExpiry: ((displayId: string) => void) | null = null;
   ...
   */
  init(
    displayRepo: Repository<'displays'>,
    horseRepo: Repository<'horses'>,
    onExpiry: (displayId: string) => void
  ): void {
  ...
  private loadExistingExpiries(): void {
    // Load unexpired display overrides
    const overrides = this.db
      .prepare(
        `SELECT id, id as display_id, override_until
         FROM displays
         WHERE time_mode != 'AUTO' AND override_until > datetime('now')`
      )
      .all() as Array<{ id: string; display_id: string; override_until: string }>;
    ...
    const notes = this.db
      .prepare(
        `SELECT id, display_id, note_expiry
         FROM horses
         WHERE note IS NOT NULL AND note_expiry > datetime('now')`
      )
      .all() as Array<{ id: string; display_id: string; note_expiry: string }>;
  ...
  private handleExpiry(record: ExpiryRecord): void {
    if (!this.displayRepo || !this.horseRepo || !this.onExpiry) {
    ...
    try {
      if (record.type === 'override') {
        this.displayRepo.update({ timeMode: 'AUTO', overrideUntil: null }, record.id);
        console.log(`[Scheduler] Reset override for display ${record.id}`);
      ...
      // Notify via SSE
      this.onExpiry(record.displayId);
    } catch (err) {
