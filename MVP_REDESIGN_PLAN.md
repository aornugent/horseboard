# MVP Redesign Implementation Plan

## Overview

This plan outlines the implementation of a simplified web-first architecture for the Dynamic Information Board MVP.
...
**POST /api/pair** (Pair with display)
```json
{
  "success": true,
  "displayId": "d_abc123"
...
If preserving any current code:

1. **Backend logic** - `backend/server.js` route handlers can inform new `server/services/display.js`
2. **Table rendering** - `google-tv-app/.../table_display.html` styles/JS can be adapted
3. **Table editor UI** - `mobile-app/.../TableEditorScreen.js` component logic can guide new implementation

The current implementations serve as reference but the new codebase should be built fresh for cleaner architecture.
